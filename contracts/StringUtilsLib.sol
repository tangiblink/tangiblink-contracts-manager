...
library StringUtilsLib {
  ...
  function split(
    ...
    while (i < nt.length) {
      ...
      uint256 count = 0;
      for (uint256 a = i; a < ((nc.length > (nt.length - i)) ? (nt.length - i) : (i + nc.length)); a++) {
        findBytes[count] = bytes(text)[a];
        count++;
      }
      string memory find = string(findBytes);
      if (keccak256(abi.encodePacked(find)) == keccak256(abi.encodePacked(char))) {
   ...
   */
  function includes(string memory text, string memory search) internal pure returns (bool) {
    bytes memory nt = bytes(text);
    ...
    for (uint256 i = 0; i < nt.length; i++) {
      ...
      string memory find = string(findBytes);
      if (keccak256(abi.encodePacked(find)) == keccak256(abi.encodePacked(search))) {
        exist = true;
        break;
      }
    }
  ...
  function replaceOne(
    ...
    for (uint256 i = 0; i < nt.length; i++) {
      ...
      string memory find = string(findBytes);
      if (keccak256(abi.encodePacked(find)) == keccak256(abi.encodePacked(replace))) {
        bytes memory findBytes1 = new bytes((nt.length - nc.length) + nrt.length);
        uint256 count1 = 0;
  ...
  function replaceAll(
    ...
    while (i < nt.length) {
      ...
      if (keccak256(abi.encodePacked(find)) == keccak256(abi.encodePacked(replace))) {
        ...
        replacedString = string.concat(replacedString, string(findBytes1));
        i += nc.length;
        pointA = i;
      } else {
        i += 1;
   ...
   */
  function matchStr(string memory text, string memory matchtext) internal pure returns (bool exist, int256 start) {
    bytes memory nt = bytes(text);
    ...
    for (uint256 i = 0; i < nt.length; i++) {
      ...
      string memory find = string(findBytes);
      if (keccak256(abi.encodePacked(find)) == keccak256(abi.encodePacked(matchtext))) {
        matchExist = true;
        matchStart = count1;
        break;
      }
   ...
   */
  function toLowerCase(string memory text) internal pure returns (string memory) {
    string memory charsString = "Aa;Bb;Cc;Dd;Ee;Ff;Gg;Hh;Ii;Jj;Kk;Ll;Mm;Nn;Oo;Pp;Qq;Rr;Ss;Tt;Uu;Vv;Ww;Xx;Yy;Zz";
   ...
   */
  function toUpperCase(string memory text) internal pure returns (string memory) {
    string memory charsString = "Aa;Bb;Cc;Dd;Ee;Ff;Gg;Hh;Ii;Jj;Kk;Ll;Mm;Nn;Oo;Pp;Qq;Rr;Ss;Tt;Uu;Vv;Ww;Xx;Yy;Zz";
    string[] memory chars = split(charsString, ";", 40);
    bytes memory nt = bytes(text);
   ...
   */
  function repeat(string memory text, uint256 repeatLength) internal pure returns (string memory) {
  ...
  function padStart(
    ...
    bytes memory nt = bytes(text);
    for (uint256 i = 0; i < ((lengthCount > nt.length ? lengthCount : nt.length) - nt.length); i++) {
   ...
   */
  function isEqual(string memory text, string memory compare) internal pure returns (bool) {
    return (keccak256(abi.encodePacked(text)) == keccak256(abi.encodePacked(compare)));
  }
   ...
   * @return A bool which is true if the text is equal
   */
  function isEqualCase(string memory text, string memory compare) internal pure returns (bool) {
    return ((keccak256(abi.encodePacked(toUpperCase(text))) == keccak256(abi.encodePacked(toUpperCase(compare)))) &&
      (keccak256(abi.encodePacked(toLowerCase(text))) == keccak256(abi.encodePacked(toLowerCase(compare)))));
  }
   ...
   */
  function substring(string memory text, uint256 start, uint256 end) internal pure returns (string memory) {
   ...
   */
  function trimStart(string memory text) internal pure returns (string memory) {
    bytes memory nt = bytes(text);
    uint256 i = 0;
    uint256 count = 0;
    while (i < nt.length) {
   ...
   */
  function charAt(string memory text, uint256 index) internal pure returns (string memory) {
    bytes memory findBytes = new bytes(1);
   ...
   */
  function indexOf(string memory text, string memory char) internal pure returns (uint256) {
    bytes memory nt = bytes(text);
    uint256 count = 0;
    for (uint256 i = 0; i < nt.length; i++) {
      bytes memory findBytes = new bytes(1);
   ...
   * @return the index of the character.
   */
  function lastIndexOf(string memory text, string memory char) internal pure returns (uint256) {
    bytes memory nt = bytes(text);
    uint256 count = 0;
    for (uint256 i = 0; i < nt.length; i++) {
      bytes memory findBytes = new bytes(1);
   ...
   */
  function startsWith(string memory text, string memory start) internal pure returns (bool) {
    bytes memory nt = bytes(text);
   ...
   */
  function endsWith(string memory text, string memory end) internal pure returns (bool) {
    bytes memory nt = bytes(text);
